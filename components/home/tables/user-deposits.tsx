    ...
    return (
        <ChartWrapper>
            <Box w="100%" mx="2" display="flex" justifyContent={"space-between"}>
                <Text fontSize="lg" fontWeight="semibold">Largest User Deposits By USD Value</Text>
            </Box>
                    ...
                    {headerGroups.map((headerGroup, i) => (
                        <Tr {...headerGroup.getHeaderGroupProps()} key={i}>
                            {headerGroup.headers.map((column, j) => (
                                <Th color="#fff" {...column.getHeaderProps()} key={j}>{column.render('Header')}</Th>
                            ))}
                        </Tr>
                    ))}
                </Thead>
            ...
            >
                <Box display="flex">
                    <ButtonGroup isAttached={true}>
                        <Button variant="primary" size="sm" onClick={() => gotoPage(0)} disabled={!canPreviousPage}>
                            {"First"}
                        </Button>
                        <Button variant="primary" size="sm" onClick={() => previousPage()} disabled={!canPreviousPage}>
                            {"<"}
                        </Button>
                        <Button variant="primary" size="sm" onClick={() => nextPage()} disabled={!canNextPage}>
                            {">"}
                        </Button>
                        <Button variant="primary" size="sm" onClick={() => gotoPage(pageCount - 1)} disabled={!canNextPage}>
                            {"Last"}
                        </Button>
                    </ButtonGroup>
                </Box>
